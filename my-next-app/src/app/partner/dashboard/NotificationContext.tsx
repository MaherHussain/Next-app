...
interface NotificationContextType {
  ...
  setAllNotifications: (orders: NotificationOrder[]) => void;
...
export const NotificationProvider = ({ children }: { children: ReactNode }) => {
  ...
  const setAllNotifications = (orders: NotificationOrder[]) => {
    ...
    if (orders.length > 0) startNotificationSound();
  ...
  const removeNotification = (orderId: string) => {
    setNotifications((prev) =>
      prev.filter((notification) => notification._id !== orderId)
    );
  };
  ...
  }, [socket, user?.restaurantId, notifications]);
