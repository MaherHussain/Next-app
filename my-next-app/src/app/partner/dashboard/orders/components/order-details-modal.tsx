...
import { getOneOrder, useAcceptOrder, useRejectOrder } from '@/app/queries/orders';
...
function OrderDetailsModal({ isOpen, onClose, orderId }: OrderDetailsModalProps) {
  ...
  const handleReject = async () => {
    if (!orderId) return;
    try {
      await rejectOrder({ orderId, rejectionReason: rejectionReason || "Restaurant cannot fulfill" });
      queryClient.invalidateQueries({ queryKey: ['orders'] });
      queryClient.invalidateQueries({ queryKey: ['order', orderId] });
      onClose();
    } catch (error) {
